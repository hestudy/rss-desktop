...
/// 获取 RSSHub Radar 规则
#[tauri::command]
pub async fn get_rsshub_radar_rules() -> CommandResult<crate::rsshub::RadarRules> {
    let client = crate::rsshub::RSSHubClient::default();
    client.get_radar_rules()
}

/// 从 URL 检测可用的 RSSHub 订阅
#[tauri::command]
pub async fn detect_rsshub_feeds(
    url: String,
    instance_url: Option<String>,
) -> CommandResult<Vec<crate::rsshub::DetectedFeed>> {
    let instance = instance_url.unwrap_or_else(|| crate::rsshub::DEFAULT_RSSHUB_INSTANCE.to_string());
    let client = crate::rsshub::RSSHubClient::new(instance);
    client.detect_feeds(&url)
}

/// 搜索 RSSHub 路由
#[tauri::command]
pub async fn search_rsshub_routes(
    query: String,
) -> CommandResult<Vec<crate::rsshub::RouteInfo>> {
    let client = crate::rsshub::RSSHubClient::default();
    client.search_routes(&query)
}

/// 测试 RSSHub 实例连接
#[tauri::command]
pub async fn test_rsshub_connection(
    instance_url: String,
) -> CommandResult<bool> {
    let client = crate::rsshub::RSSHubClient::new(instance_url);
    client.test_connection()
}

/// RSS URL 验证结果
#[derive(Debug, Clone, serde::Serialize)]
pub struct RssUrlValidationResult {
    /// 是否有效
    pub valid: bool,
    /// 错误类型
...
/// 验证 RSS URL 是否有效
#[tauri::command]
pub async fn validate_rss_url(url: String) -> CommandResult<RssUrlValidationResult> {
    let url_clone = url.clone();

    // 在阻塞任务中执行网络请求
    let result = tauri::async_runtime::spawn_blocking(move || {
        match fetch_feed(&url_clone) {
            Ok((feed, _articles)) => {
                // 成功获取 feed
                Ok(RssUrlValidationResult {
                    valid: true,
                    error_type: None,
                    error_message: None,
                    feed_info: Some(RssFeedInfo {
                        title: feed.title,
            ...
            Err(e) => {
                ...
                let (error_type, error_message) = if error_str.contains("403") || error_str.contains("forbidden") {
                    ("forbidden".to_string(), "该路由被限制访问 (403)".to_string())
                } else if error_str.contains("404") || error_str.contains("not found") {
                    ("not_found".to_string(), "订阅地址不存在 (404)".to_string())
                } else if error_str.contains("timeout") || error_str.contains("timed out") {
                    ("timeout".to_string(), "请求超时".to_string())
                } else if error_str.contains("network") || error_str.contains("connection") || error_str.contains("connect") {
                    ("network".to_string(), "网络连接失败".to_string())
                } else if error_str.contains("parse") || error_str.contains("invalid") || error_str.contains("xml") {
                    ("invalid_rss".to_string(), "无效的 RSS 格式".to_string())
                } else {
