 ...
 * 默认 RSSHub 设置
 */
export const DEFAULT_RSSHUB_SETTINGS: RSSHubSettings = {
  instanceUrl: 'https://rsshub.app',
  enabled: true,
}

/**
 * Zod schema for RSSHubSettings validation
 */
export const RSSHubSettingsSchema = z.object({
  instanceUrl: z.string().url('请输入有效的 URL').default('https://rsshub.app'),
 ...
 * 获取 RSSHub 设置
 */
export async function getRSSHubSettings(): Promise<RSSHubSettings> {
  const result = await invoke<RSSHubSettings>('get_rsshub_settings')
  return RSSHubSettingsSchema.parse(result)
}

/**
 * 更新 RSSHub 设置
 */
export async function updateRSSHubSettings(settings: Partial<RSSHubSettings>): Promise<RSSHubSettings> {
  const current = await getRSSHubSettings()
  const merged = { ...current, ...settings }
  const validated = RSSHubSettingsSchema.parse(merged)
  const result = await invoke<RSSHubSettings>('update_rsshub_settings', { settings: validated })
  return RSSHubSettingsSchema.parse(result)
}
