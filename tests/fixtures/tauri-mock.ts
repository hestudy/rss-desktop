    ...
    switch (command) {
      ...
      case 'get_rsshub_radar_rules':
        return Promise.resolve({
          'bilibili': [
            {
              title: 'UP 主视频',
              docs: 'https://docs.rsshub.app/routes/bilibili#up-zhu-shi-pin',
              source: '/space\\\\.bilibili\\\\.com/(\\\\d+)',
              target: '/bilibili/user/video/:uid',
            },
            {
              title: 'UP 主动态',
              docs: 'https://docs.rsshub.app/routes/bilibili#up-zhu-dong-tai',
              source: '/space\\\\.bilibili\\\\.com/(\\\\d+)',
          ...
          'youtube': [
            {
              title: '用户视频',
              docs: 'https://docs.rsshub.app/routes/youtube#yong-hu',
              source: '/youtube\\\\.com/@([^/]+)',
          ...
          'github': [
            {
              title: '仓库 Issues',
              docs: 'https://docs.rsshub.app/routes/github#issues',
              source: '/github\\\\.com/([^/]+)/([^/]+)',
              target: '/github/issue/:owner/:repo',
            },
          ],
        });
      ...
      case 'detect_rsshub_feeds': {
        ...
        if (url.includes('bilibili.com')) {
          ...
          const uid = match ? match[1] : '123456';
          detectedFeeds.push({
            namespace: 'bilibili',
            title: 'UP 主视频',
            rss_url: 'https://rsshub.app/bilibili/user/video/' + uid,
            docs: 'https://docs.rsshub.app/routes/bilibili',
            source_url: url,
        ...
        if (url.includes('youtube.com')) {
          detectedFeeds.push({
            namespace: 'youtube',
            title: '用户视频',
            rss_url: 'https://rsshub.app/youtube/user/testuser',
            docs: 'https://docs.rsshub.app/routes/youtube',
            source_url: url,
            requires_params: false,
          });
        }

        return Promise.resolve(detectedFeeds);
      }
      ...
      case 'search_rsshub_routes': {
        ...
        const allRoutes = [
          ...
          {
            path: '/github/issue/:owner/:repo',
            namespace: 'github',
            title: '仓库 Issues',
            docs: 'https://docs.rsshub.app/routes/github',
            required: ['owner', 'repo'],
          },
          {
        ...
        const filtered = allRoutes.filter(route =>
          route.namespace.includes(query) ||
